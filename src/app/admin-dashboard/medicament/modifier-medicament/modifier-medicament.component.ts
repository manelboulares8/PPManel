...
export class ModifierMedicamentComponent implements OnInit {
  ...
  constructor(
    ...
    private servicceComponent: ServicceComponent,
    private formBuilder: FormBuilder
  ) {}

  ngOnInit(): void {
    const id = this.activatedRoute.snapshot.params['id']; // Récupérer l'ID de la route
    this.newMedicament = this.servicceComponent.consulterMedicament(id); // Remplir `newMedicament` avec les données récupérées
 ...
 /* modifierMedicament() {
    if (this.myForm.valid) {
      this.servicceComponent.modifierMed(this.newMedicament);
